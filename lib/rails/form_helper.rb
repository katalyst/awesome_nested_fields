...
protected
  ...
  def render_nested_fields_template(association, options, &block)
    templates = @template.content_tag(:div, :style => 'display:none;', :class => options[:item_template_class]) do
    ...
    if options[:show_empty]
      empty_template = @template.content_tag(:div, :style => 'display:none;', :class => options[:empty_template_class]) do
